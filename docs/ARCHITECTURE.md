...
正式 v2 只有一条运行链路：`miniprogram/` 原生微信小程序通过 REST API 接入 `backend/` FastAPI。小程序延续紫色主题、自定义底部导航、空间导览和双页预约交互。
...
旧 uni-app 与微信云函数实现已从工作树移除并保留在 Git 历史中。后续开发不得重新建立平行前端或后端。微信开发者工具应直接导入仓库根目录，由根目录 `project.config.json` 的 `miniprogramRoot: "miniprogram/"` 加载正式前端。
...
正式 v2 不依赖微信云数据库、云存储或云函数；主数据只写入 FastAPI 所连接的数据库。
...
4. 当前不交付 H5；如学校确需 H5，应单独立项并与原生端业务同步。
