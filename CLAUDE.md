...
| `../dut-duxue-space-booking-showcase.design/` | 布局与交互参考（HTML 静态原型） | 视觉参考，非运行代码 |
...
`miniprogram/app.wxss` 是正式小程序的色值唯一源；外部静态原型仅用于布局与交互参考：

| 角色 | 色值 | 用途 |
|---|---|---|
| 品牌主色（清华紫） | `#612276` | 主按钮、FAB、选中态、CTA、导航选中 |
| 辅助底色（米白） | `#F8F6FA` | 页面底层、浅提示区；卡片保持纯白以形成层次 |
| 辅助紫 | `#8C6498` | 渐变端点和低强调装饰，不替代主色 |
...
- `miniprogram/` 品牌色已统一为清华紫 `#612276` + 米白 `#F8F6FA`，色值变量集中在 `app.wxss`，旧橙紫或旧灰紫复现由守护拦截【守护: backend/guards/test_conventions.py】
...
- UI 风格：`miniprogram/app.wxss` 是色值唯一源（清华紫 `#612276` 主 + 米白 `#F8F6FA` 辅），外部静态原型仅作布局与交互参考；状态语义色（成功绿/danger 红/cleanup 棕/info 蓝）保留不动
